...
#M# versal-2ve-2vm-vek385-sdt-seg --add-config\ CONFIG_YOCTO_BBMC_CORTEXR52_0_ZEPHYR=y\ --add-config\ CONFIG_YOCTO_BBMC_CORTEXR52_1_BAREMETAL=y\ --add-config\ CONFIG_YOCTO_BBMC_MICROBLAZE_RISCV_ASU=disable  full    openamp-overlay-versal-2ve-2vm.yaml   none    QEMU_HW_DTB_PS = \"\${QEMU_HW_DTB_PATH}/board-versal2-psxc-vek385.dtb\"\\nQEMU_HW_BOOT_MODE = \"8"\\nQEMU_HW_SERIAL = \"-serial null -serial null -serial null -serial mon:stdio\"\\nQEMU_HW_OSPI_FILE = \"qemu-ospi-${MACHINE}.bin\"\\nQB_OPT_APPEND:append := \" -drive file=@DEPLOY_DIR_IMAGE@/\${QEMU_HW_OSPI_FILE},if=mtd,format=raw,index=0\"\\nQB_DEFAULT_FSTYPE = \"wic.ufs\"\\nQB_ROOTFS_OPT = \"-device scsi-hd,drive=d1,bus=scsi.0,channel=0,scsi-id=0,lun=0,logical_block_size=4096,physical_block
